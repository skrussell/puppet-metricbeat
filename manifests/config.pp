...
class metricbeat::config inherits metricbeat {
  ...
  # Create modules.d files that exist in hiera as virtual resources, then realise all virtualised modules
    ...
    'Linux': {
      $validate_cmd = $metricbeat::disable_configtest ? {
        ...
        default => $metricbeat::major_version ? {
          ...
          default => "/usr/share/metricbeat/bin/metricbeat -c % test config",
        }
      }
